...
(defun c-c++/post-init-flycheck ()
  ...
  (when c-c++-enable-clang-support
    (spacemacs/add-to-hooks 'c-c++/load-clang-args-c '(c-mode-hook)) ;; c++-mode-hook))
    (spacemacs/add-to-hooks 'c-c++/load-clang-args-cpp '(c++-mode-hook))
    (spacemacs/add-to-hooks '(lambda()
                               (setq-local flycheck-clang-language-standard "gnu99"
                                     ))
                            '(c-mode-hook))
    ))
